      ...
      <View style={[CommonStyles.alignCenterRow, styles.focused]}>
        <View
          style={[
            isFocused
              ? searchType === 'normal'
                ? styles.focusedNormal
                : styles[searchType]
              : null,
          ]}>
          <View style={styles.searchBar}>
            {searchType === 'transparent' ? (
              <Search style={styles.transparentIcon} />
            ) : (
              <Search style={styles.normalIcon} />
            )}
            <TextInput
              style={{height: '50%'}}
              placeholder="Search"
        ...
        <Text style={TypographyStyles.RegularNoneRegular}>
          {' '}
          {isFocused ? 'Cancel' : null}
        </Text>
      </View>
    </View>
  );
